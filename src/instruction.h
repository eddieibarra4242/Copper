...
#define OP_MEM(loc) ((Operand){.type = OPERAND_MEMORY, .value.mem_addr = (uint64_t)(loc)})
#define OP_REG(num) ((Operand){.type = OPERAND_REGISTER, .value.reg = (uint64_t)(num)})
#define OP_CONST(val) ((Operand){.type = OPERAND_CONSTANT, .value.constant = (uint64_t)(val)})
 ...
 *
 * @param list Pointer to the instruction list where the instruction will be added.
 ...
 */
Instruction *append_instruction(InstructionList *list, enum InstructionSet opcode, Operand dest, Operand src1, Operand src2);
 ...
 */
InstructionList *append_instruction_list(InstructionList *dest, InstructionList *appendage);
